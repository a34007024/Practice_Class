...
// 顯示資料
// 			 S0 S1 - M0 M1 -  H0 H1
char DISP[8]={0, 0,11,0,0,11,2, 1};// 初值為12-00-00 
	...
	{	while(_mode)			
		// ===== 調整模式 =====
 		{	TR0=0;					// 停止計時
			...
			// =======================
			if(!_H && _Shift)		// _H
			{	if (DISP[7]==1 && DISP[6]==2)// 23時
				{	DISP[7]=0;DISP[6]=1;}		// 調回00時
			...
			{	if (DISP[7]==0 && DISP[6]==1)	// 00時
				{	DISP[7]=1;DISP[6]=2;}		// 調回23時
		...
		{	if (DISP[1]<5) 
			...
			{ 	if (DISP[3]<9) 
				...
				{ 	if (DISP[4]<5) 
					...
					// 分十位數進位
					{ 	if (DISP[7]==1 && DISP[6]==2) 
						{	DISP[7]=0;DISP[6]=1;// 時歸零 
							DISP[4]=DISP[3]=0;// 分歸零 
							DISP[1]=DISP[0]=0;// 秒歸零 
						}
						else if (DISP[6]<9) 
